...
namespace UnitTests
{
    TEST_CLASS(TypeTraitsTest)
    {
        ...
        {
            Assert::AreEqual<size_t>(3, arcana::count_true<std::true_type, std::true_type, std::true_type>::value);
            Assert::AreEqual<size_t>(0, arcana::count_true<>::value);
            Assert::AreEqual<size_t>(1, arcana::count_true<std::true_type>::value);
            Assert::AreEqual<size_t>(0, arcana::count_true<std::false_type>::value);
            Assert::AreEqual<size_t>(2, arcana::count_true<std::true_type, std::false_type, std::true_type>::value);
        }

        TEST_METHOD(FindFirstTrueConditionalExpressionIndex)
        {
            Assert::AreEqual<size_t>(0, arcana::find_first_index<std::true_type, std::true_type, std::true_type>::value);
            Assert::AreEqual<size_t>(1, arcana::find_first_index<std::false_type, std::true_type, std::true_type>::value);
            Assert::AreEqual<size_t>(1, arcana::find_first_index<std::false_type, std::true_type>::value);
            Assert::AreEqual<size_t>(0, arcana::find_first_index<std::true_type, std::false_type>::value);
            Assert::AreEqual<size_t>(2, arcana::find_first_index<std::false_type, std::false_type, std::true_type>::value);

            // test the end condition
            Assert::AreEqual<size_t>(0, arcana::find_first_index<>::value);
            Assert::AreEqual<size_t>(3, arcana::find_first_index<std::false_type, std::false_type, std::false_type>::value);
        }
    };
}
